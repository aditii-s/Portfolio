...
date_default_timezone_set('Asia/Kolkata');
$admremarkdate=date('Y-m-d G:i:s ', strtotime("now"));
$sql="update tblleaves set PrincipalRemark=:description,Status=:status,PrincipalRemarkDate=:admremarkdate where id=:did";
$query = $dbh->prepare($sql);
...
echo htmlentities($result->PrincipalRemark);
}
?></td>
 </tr>
...
<td colspan="5"><?php
if($result->PrincipalRemarkDate==""){
  echo "NA";  
}
else{
echo htmlentities($result->PrincipalRemarkDate);
}
?></td>
 </tr>
<?php 
